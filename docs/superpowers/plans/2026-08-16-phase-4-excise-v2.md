# Phase 4: Excise the V2 Session Runtime Implementation Plan — ✅ COMPLETE (2026-08-17)
...
- [x] **Step 1: grep-verify every deletion target** (print evidence per file group in your report)
- [x] **Step 2: delete sources; fix residual compile errors** (expect fallout in `location-services.ts` leftovers, `core/package.json` exports `./session/runner` + `./system-context`, `test/` imports)
- [x] **Step 3: rework the two opencode test helpers** (SessionV2+noopLayer read-back → direct `SessionMessageTable` queries via the keep-list `Database`)
- [x] **Step 4: verify** — `cd packages/core && bun typecheck && bun test`; `cd packages/opencode && bun typecheck && bun test test/session test/server`; record remaining failures (must be a subset of the documented pre-existing set: prompt.test.ts ×2, test/server ×7, test/cli orphans)
- [x] **Step 5: commit** — `refactor(core)!: delete the V2 session runtime` (message body: line count removed)
...
- [x] **Step 1: delete the TUI dead v2 calls** — `packages/tui/src/context/data.tsx:422-459` (the five never-invoked refresh functions and any store fields that become unused — verify with greps before removing fields; `useData`'s only live consumer is `autocomplete.tsx:93` reading `data.location.reference.list`)
- [x] **Step 2: SDK regen** — run the generator (root `script/generate.ts` per this repo's layout); inspect the diff: expected = v2 `/api/session*` etc. operations vanish from `sdk.gen.ts`/`types.gen.ts`, plus the known pre-existing openapi.json drift. Remove the `V2SessionHistoryData` post-processing regex in `packages/sdk/js/script/build.ts:79` if present. Commit generated files SEPARATELY: `chore(sdk): regenerate after V2 API removal`.
- [x] **Step 3: typecheck** `packages/tui`, `packages/opencode`; run `bun test test/cli` in opencode (watch the help-snapshot test — command set changed when `v2` was removed; update snapshot if it's managed in-repo)
- [x] **Step 4: commit** hand edits: `refactor(tui): remove dead v2 session data calls`

---

## Task 4.5: docs + Phase 4 verification gate

- [x] **Step 1: docs** — root `AGENTS.md`: delete the "V2 Session Core" section + the stale `packages/client` bullet. Root `CONTEXT.md`: replace V2 vocabulary with a short V1-architecture description; keep the good ideas (durable prompt admission, provider-turn boundaries, context epochs) as "possible future ports" notes in the surgery plan's Phase 5 list. Update the main plan's Phase 4 section with results.
- [x] **Step 2: full verification** — typecheck core/opencode/tui/ui; `bun test` suites in core+opencode (failures must be subset of documented pre-existing); `bun run script/bench-rss.ts` (expect `--help` visibly below 144 MB).
...
## Verification results (2026-08-17, all gates passed)
...
- CLI bench: `--help` 144 MB (flat — the deleted graph was already behind dynamic imports in the CLI path)
- Binary size: 142 MB (unchanged at MB granularity)

**Structure (the real payoff):** −18k lines, −2 packages (`@opencode-ai/protocol`, `@opencode-ai/server`), one session runtime instead of two, all public routes owned by the instance HttpApi, SDK regenerated from the true public surface (61 → 12 v2 operations). AGENTS.md/CONTEXT.md rewritten to the V1 reality.

**Smoke (compiled `0.0.0-memory-surgery-202608170114`):** TUI boots; prompt cycle healthy; `v2` gone from `--help`, `serve`/`attach` present; TUI kill → server survives; relaunch `--continue` → same serve pid reused, session restored; zero orphan processes after cleanup.

**Test suites:** core 781/16 + opencode 704/18 — every failure classified as documented pre-existing (provider-env, ModelsDev catalog, watcher env, fork-rename drift, acp/web orphans). Two hidden pre-existing bugs found and fixed along the way: test preloads setting the old `OPENCODE_DB` (tests hit the real user DB), location-layer test writing `opencode.json` (Config only reads `openchinacode.json`).
