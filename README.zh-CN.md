...
- 原生接入火山方舟 Seedream 5 Pro 生图和 Seedance 2.0 Mini 生视频。

完整功能手册见 [manual.md](./manual.md)。
...
安装脚本会从 GitHub Releases 下载匹配当前系统的二进制，并默认安装为 `~/.local/bin/openchinacode`。可以用 `--install-dir <dir>` 指定其它目录，也可以用 `--no-modify-path` 禁止脚本修改 shell 配置文件。

GitHub raw URL 仍可作为备用安装入口：
...
具体用法见 [manual.md](./manual.md)。

## 原生生图 / 生视频

OpenChinaCode 现在可以通过火山方舟生成媒体：

- 图片：`image_generate` 工具，Seedream 5 Pro 模型 `doubao-seedream-5-0-pro-260628`
- 视频：`video_generate` / `video_status` 工具，Seedance 2.0 Mini 模型 `doubao-seedance-2-0-mini-260615`
...
- 自然语言：直接让 OpenChinaCode 生成图片或视频；如果关键参数不明确，它应该先追问，再调用原生工具。
- Slash 向导：输入 `/image-generate` 或 `/video-generate`，进入参数选择 / 输入流程。
- 快速 slash：`/image-generate 一只赛博朋克猫 mascot` 或 `/video-generate 一个 5 秒项目宣传短片`。
