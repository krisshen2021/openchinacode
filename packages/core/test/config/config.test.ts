...
describe("Config", () => {
  it.effect("returns the latest defined scalar from priority-ordered documents", () =>
    Effect.sync(() => {
      const entries = [
        new Config.Document({ type: "document", info: new Config.Info({ model: "zhipuai-pay2go/glm-5.2" }) }),
        new Config.Directory({ type: "directory", path: AbsolutePath.make("/skills") }),
        new Config.Document({ type: "document", info: new Config.Info({}) }),
        new Config.Document({ type: "document", info: new Config.Info({ model: "deepseek/deepseek-v4-pro" }) }),
      ]

      expect(Config.latest(entries, "model")).toBe("deepseek/deepseek-v4-pro")
  ...
  it.effect("parses and migrates integration_test configuration", () =>
    Effect.sync(() => {
      ...
      const migrated = Schema.decodeUnknownSync(Config.Info)(ConfigMigrateV1.migrate(v1), { errors: "all" })

      expect(migrated.integration_test?.base_url).toBe("http://127.0.0.1:5173")
      expect(migrated.integration_test?.frontend?.port).toBe(5173)
      expect(migrated.integration_test?.mcp?.timeout).toBe(30000)
    }),
  )
  ...
  it.live("loads openchinacode JSON and JSONC files from lowest to highest priority", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(
                path.join(tmp.path, "openchinacode.json"),
                JSON.stringify({ $schema: "base", providers: { base: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "openchinacode.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(documents[0]?.path).toBe(path.join(tmp.path, "openchinacode.json"))
            expect(documents[1]?.info.providers?.last).toBeInstanceOf(ConfigProvider.Info)

            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "openchinacode.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
  ...
  it.live("accepts $schema metadata without writing it into config files", () =>
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          const file = path.join(tmp.path, "openchinacode.json")
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(reviewer?.model).toBe("zhipuai-pay2go/glm-5.2")
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(path.join(tmp.path, "openchinacode.json"), JSON.stringify({ $schema: "base" })),
              fs.writeFile(path.join(tmp.path, "openchinacode.jsonc"), "{ invalid"),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
      ...
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.writeFile(
              path.join(global, "openchinacode.json"),
              JSON.stringify({
                experimental: { policies: [{ effect: "deny", action: "provider.use", resource: "openai" }] },
              }),
            )
  ...
  it.live("loads global, ancestor, and .openchinacode configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".openchinacode"), { recursive: true })
            await fs.mkdir(path.join(directory, ".openchinacode"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "openchinacode.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "openchinacode.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "openchinacode.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "openchinacode.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "openchinacode.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(
                path.join(root, ".openchinacode", "openchinacode.json"),
                JSON.stringify({ $schema: "root-dot" }),
              ),
              fs.writeFile(
                path.join(directory, ".openchinacode", "openchinacode.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".openchinacode")),
              AbsolutePath.make(path.join(directory, ".openchinacode")),
            ])
