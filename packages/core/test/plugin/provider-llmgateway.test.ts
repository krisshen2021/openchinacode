  ...
  it.effect("applies legacy referer headers only to enabled llmgateway", () =>
    Effect.gen(function* () {
      ...
      yield* integrations.transform((editor) => {
        ...
        editor.update(Integration.ID.make("control-provider"), () => {})
      })
      yield* catalog.transform((catalog) => {
        ...
        catalog.provider.update(ProviderV2.ID.make("control-provider"), () => {})
      })
      yield* addPlugin()
      ...
      expect((yield* catalog.provider.get(ProviderV2.ID.make("control-provider")))?.request.headers).toEqual({})
    }),
  )
