...
describe("BashTool", () => {
  ...
  it.live("blocks broad process-kill commands before approval or execution", () =>
    Effect.acquireUseRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => {
        reset()
        return withTool(tmp.path, (registry) =>
          settleTool(registry, call({ command: 'pkill -f "tsx"' }, "call-block-kill")),
        ).pipe(
          Effect.andThen((settled) =>
            Effect.sync(() => {
              ...
              expect(settled.output?.content).toEqual([
                {
                  type: "text",
                  text: expect.stringContaining("process_stop"),
                },
              ])
            }),
          ),
  ...
  it.live("blocks long-running service commands before approval or execution", () =>
    Effect.acquireUseRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => {
        reset()
        return withTool(tmp.path, (registry) =>
          settleTool(registry, call({ command: "npx tsx server/index.ts" }, "call-block-server")),
        ).pipe(
          Effect.andThen((settled) =>
            Effect.sync(() => {
              expect(assertions).toEqual([])
              ...
              expect(settled.output?.content).toEqual([
                {
                  type: "text",
                  text: expect.stringContaining("process_start"),
                },
              ])
            }),
          ),
...
test("keeps locked deferred parity TODOs visible", async () => {
  ...
  for (const todo of [
    ...
    "Persist managed process status and define restart recovery before exposing remote observation.",
