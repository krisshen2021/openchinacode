...
import { ConfigIntegrationTest } from "./config/integration-test"
import { ConfigLSP } from "./config/lsp"
import { ConfigMCP } from "./config/mcp"
import { ConfigPlugin } from "./config/plugin"
import { ConfigProvider } from "./config/provider"
import { ConfigReference } from "./config/reference"
import { ConfigTaskPolicy } from "./config/task-policy"
import { ConfigToolOutput } from "./config/tool-output"
import { ConfigWatcher } from "./config/watcher"
import { ConfigV1 } from "./v1/config/config"
import { ConfigMigrateV1 } from "./v1/config/migrate"

const AutoMaxTokens = Schema.Union([
  Schema.Boolean,
  Schema.Literals(["off", "heuristic", "llm"]),
  Schema.Struct({
    mode: Schema.optional(Schema.Literals(["off", "heuristic", "llm"])),
...
export class Info extends Schema.Class<Info>("Config.Info")({
  ...
  auto_maxtokens: AutoMaxTokens.pipe(Schema.optional),
  task_policy: ConfigTaskPolicy.Info.pipe(Schema.optional).annotate({
    description: "OpenChinaCode subagent task classification and task-to-model routing policy.",
  }),
  integration_test: ConfigIntegrationTest.Info.pipe(Schema.optional).annotate({
    description: "OpenChinaCode frontend/backend integration test kit configuration.",
  }),
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const names = ["openchinacode.json", "openchinacode.jsonc"]
    ...
    const discovered = locationIsGlobal
      ...
      : yield* fs
          .up({
            targets: [".openchinacode", ...names.toReversed()],
    ...
    const directories = [
      globalDirectory,
      ...discovered
        .filter((item) => path.basename(item) === ".openchinacode")
        .toReversed()
        .map((directory) => AbsolutePath.make(directory)),
    ]
    // A config closer to the opened directory should win over one higher up.
    // Search starts nearby, so reverse the results before applying them.
    const directPaths = discovered.filter((item) => path.basename(item) !== ".openchinacode").toReversed()
    ...
    // global config, project files, then `.openchinacode` files.
