import type {
  ...
  SnapshotFileDiff,
  ConsoleState,
  EventSessionCompactionProgress,
...
export type CompactionProgressItem = EventSessionCompactionProgress["properties"] & {
  id: string
  time: number
}
...
export const {
  ...
  init: () => {
    ...
    const [store, setStore] = createStore<{
      ...
      compaction_progress: {
        [sessionID: string]: CompactionProgressItem[]
      }
      lsp: LspStatus[]
      mcp: {
    ...
    event.subscribe((event, { directory, workspace }) => {
      switch (event.type) {
        ...
        case "session.deleted": {
          ...
          setStore("compaction_progress", event.properties.info.id, [])
          break
        }
        ...
        case "session.compaction.progress": {
          const sessionID = event.properties.sessionID
          const item: CompactionProgressItem = {
            ...event.properties,
            id: event.id,
            time: Date.now(),
          }
          const current = event.properties.stage === "started" ? [] : (store.compaction_progress[sessionID] ?? [])
          if (current.some((existing) => existing.id === event.id)) break
          setStore("compaction_progress", sessionID, [...current, item].slice(-24))
          break
        }
