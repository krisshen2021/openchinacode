...
import { useSync, type CompactionProgressItem } from "../../context/sync"
          ...
          <box flexGrow={1} minHeight={0} paddingBottom={1} paddingLeft={2} paddingRight={2} gap={1}>
            <Show when={session()}>
              ...
              >
                ...
                <CompactionDebug items={compactionProgress()} />
              </scrollbox>
              <box flexShrink={0}>
...
function formatCompactionDebugModel(model: CompactionProgressItem["model"]) {
...
function formatCompactionDebugLine(item: CompactionProgressItem) {
  ...
  if (item.stage === "route" && model) {
    return `route: ${model} - ${item.message}`
  }

  if (item.stage === "judge_started") {
    return `judge: started${model ? ` - ${model}` : ""}`
  }

  if (item.stage === "judge_result" && item.judge) {
    const judgeModel =
    ...
    return `judge: ${judgeModel ?? "unavailable"} -> ${item.judge.status}${elapsed}${error}`
  }

  if (item.stage === "profile_ready" && item.profile) {
    const profiles = item.profile.profiles
      .map((profile) => `${profile.type} ${Math.round(profile.weight * 100)}%`)
      .join(", ")
...
function CompactionDebug(props: { items: CompactionProgressItem[] }) {
  const { theme } = useTheme()
  return (
    <Show when={props.items.length}>
      ...
      >
        <box paddingTop={1} paddingBottom={1} paddingLeft={2} backgroundColor={theme.backgroundPanel}>
          <text fg={theme.text}>
            <span style={{ fg: theme.accent, bold: true }}>Smart Compaction...</span>
          </text>
          <For each={props.items}>
            {(item) => (
              <text fg={theme.textMuted}>
                <span style={{ fg: theme.textMuted }}>{formatCompactionDebugLine(item)}</span>
              </text>
            )}
          </For>
        </box>
...
function Task(props: ToolProps) {
  ...
  const content = createMemo(() => {
    ...
    const route = formatTaskRoute(props.metadata)
...
export function formatTaskRoute(metadata: Record<string, unknown>) {
  ...
  const route = [`${providerID}/${modelID}`, stringValue(metadata.variant)].filter(Boolean).join("#")
  const policy = recordValue(metadata.taskPolicy)
  const assignment = [stringValue(policy?.kind), stringValue(policy?.complexity)].filter(Boolean).join(".")
  const source = stringValue(metadata.modelSource)
