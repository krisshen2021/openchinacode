...
export const Model = Schema.Struct({
  ...
  task_classes: optional(Schema.Array(ConfigTaskPolicy.TaskKind)),
        ...
        for (const [providerID, provider] of configProviders) {
          ...
          for (const [modelID, model] of Object.entries(provider.models ?? {})) {
            ...
            const parsedModel: Model = {
              ...
              task_classes: model.task_classes ?? existingModel?.task_classes,
              variants: {},
            }
          ...
          if (
            ...
            try {
              ...
              if (body && typeof body === "object" && !Array.isArray(body)) {
                opts.body = JSON.stringify(ProviderTransform.rewriteRequestBody(model, body))
              }
            } catch {}
          }
...
const smallModelFamilyPriority = ["glm-flash", "kimi-k2", "deepseek"]
export function sort<T extends { id: string }>(models: T[]) {
  const priorityScore = (id: string) => {
    const index = priority.findIndex((filter) => id.includes(filter))
    return index === -1 ? -1 : priority.length - index
  }
  return sortBy(
    models,
    [(model) => priorityScore(model.id), "desc"],
