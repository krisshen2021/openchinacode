...
import {
  ChinaTransform,
  type AutoMaxTokensConfig as ChinaAutoMaxTokensConfig,
  type MaxOutputDecision as ChinaMaxOutputDecision,
} from "./china-transform"
...
function normalizeMessages(
  ...
  // DeepSeek requires reasoning replay only for assistant turns that led to tool calls.
  if (model.api.id.toLowerCase().includes("deepseek")) {
    msgs = msgs.map((msg) => {
      if (msg.role !== "assistant") return msg
      if (Array.isArray(msg.content)) {
        if (!ChinaTransform.shouldPreserveReasoningForMessage(model, msg.content)) {
      ...
      if (!ChinaTransform.shouldPreserveReasoningForMessage(model, msg.content)) return msg
      return {
        ...msg,
        content: [
  ...
  if (
    ...
    return msgs.map((msg) => {
      if (msg.role === "assistant" && Array.isArray(msg.content)) {
        ...
        if (
          reasoningText === "" &&
          ChinaTransform.family(model) === "deepseek" &&
          !filteredContent.some((part: any) => part.type === "tool-call")
        ) {
          return {
            ...msg,
...
function unsupportedParts(msgs: ModelMessage[], model: Provider.Model): ModelMessage[] {
  return msgs.map((msg) => {
    ...
    const filtered = msg.content.map((part) => {
      ...
      return {
        type: "text" as const,
        text: `ERROR: Cannot read ${name} (this model does not support ${modality} input).${routeVisual || " Inform the user."}`,
      }
    })
...
export function temperature(model: Provider.Model) {
  if (ChinaTransform.isChinaModel(model)) return ChinaTransform.temperature(model)
...
export function topP(model: Provider.Model) {
  if (ChinaTransform.isChinaModel(model)) return ChinaTransform.topP(model)
...
export function topK(model: Provider.Model) {
  const china = ChinaTransform.topK(model)
  if (china !== undefined) return china
...
export function variants(model: Provider.Model): Record<string, Record<string, any>> {
  ...
  const china = ChinaTransform.variants(model)
  if (china !== undefined) return china
...
export function options(input: {
  ...
  const chinaOptions = ChinaTransform.options(input)
  if (chinaOptions) Object.assign(result, chinaOptions)
...
type MaxOutputTokensInput = {
  model: Provider.Model
  outputTokenMax?: number
  messages?: readonly unknown[]
  variant?: string
  autoMaxTokens?: ChinaAutoMaxTokensConfig
  agentMode?: string
  toolCount?: number
}

export type MaxOutputDecision = ChinaMaxOutputDecision
export type AutoMaxTokensConfig = ChinaAutoMaxTokensConfig

export function maxOutputTokens(model: Provider.Model, outputTokenMax?: number): number
export function maxOutputTokens(input: MaxOutputTokensInput): number
export function maxOutputTokens(input: Provider.Model | MaxOutputTokensInput, outputTokenMax?: number): number {
  const args = "model" in input ? input : { model: input, outputTokenMax }
  const chinaMaxOutputTokens = ChinaTransform.maxOutputTokens(args)
  if (chinaMaxOutputTokens !== undefined) return chinaMaxOutputTokens

  const fallbackOutputTokenMax = args.outputTokenMax ?? OUTPUT_TOKEN_MAX
  return Math.min(args.model.limit.output, fallbackOutputTokenMax) || fallbackOutputTokenMax
}

export function maxOutputDecision(input: MaxOutputTokensInput): MaxOutputDecision | undefined {
  return ChinaTransform.maxOutputDecision(input)
}

export function normalizeAutoMaxTokens(config?: AutoMaxTokensConfig) {
  return ChinaTransform.normalizeAutoMaxTokens(config)
}

export function rewriteRequestBody(model: Provider.Model, body: Record<string, any>): Record<string, any> {
  return ChinaTransform.rewriteRequestBody(model, body)
}
