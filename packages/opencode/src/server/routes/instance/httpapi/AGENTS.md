...
The `/api/*` groups in `groups/v2.ts` are location-scoped through `LocationMiddleware` (`middleware/location.ts`), not `WorkspaceRoutingMiddleware`: the middleware resolves the location ref from the deepObject `location[...]` query params or `x-opencode-*` headers and provides the request's cached location services from the shared `LocationServiceMap`. Handlers in `handlers/v2.ts` yield location services (for example `Location.Service`, `AgentV2.Service`) directly from that provision.
