...
function globalConfigFile() {
  const candidates = ["openchinacode.jsonc", "openchinacode.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "openchinacode.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "openchinacode.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        if (!Flag.OPENCODE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("openchinacode", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        for (const dir of directories) {
          if (dir.endsWith(".openchinacode") || dir === Flag.OPENCODE_CONFIG_DIR) {
            for (const file of ["openchinacode.json", "openchinacode.jsonc"]) {
          ...
          // Auto-discovered plugins under `.openchinacode/plugin(s)` are already local files, so ConfigPlugin.load
