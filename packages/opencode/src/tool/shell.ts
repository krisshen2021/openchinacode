    ...
    return () =>
      Effect.gen(function* () {
        ...
        return {
          description: [
            prompt.description,
            "OpenChinaCode safety: use bash only for short one-shot commands that naturally terminate. Use process_start, process_status, process_logs, and process_stop for dev servers, watchers, previews, backend servers, or any background work. Do not use raw &, nohup, setsid, disown, pkill -f, killall, xargs kill, npm run dev, or npx tsx through bash.",
          ].join("\n\n"),
