...
const BROWSER_VIDEO_EXTS = new Set([".webm", ".mp4", ".mov", ".m4v", ".mkv"])
const BROWSER_VIDEO_MIMES = new Set(["video/webm", "video/mp4", "video/quicktime", "video/x-matroska"])
...
>(
  ...
  Effect.gen(function* () {
    ...
    const run = Effect.fn("ReadTool.execute")(function* (
      ...
      if (BROWSER_VIDEO_EXTS.has(path.extname(filepath).toLowerCase()) || BROWSER_VIDEO_MIMES.has(mime)) {
        return yield* Effect.fail(
          new Error(
            `Cannot read video file: ${filepath}. Use browser telemetry, computed styles, screenshots, or pixel-diff checks instead.`,
          ),
        )
      }
