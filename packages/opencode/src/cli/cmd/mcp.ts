...
function parseCaps(value: string | undefined): PlaywrightCapability[] {
  ...
  if (value === "all") return [...ALL_PLAYWRIGHT_CAPS]
  const allowed = new Set<string>(ALL_PLAYWRIGHT_CAPS)
  const result = value
    .split(",")
    .map((item) => item.trim())
    .filter(Boolean)
  const invalid = result.filter((item) => !allowed.has(item))
  if (invalid.length) throw new Error(`Invalid Playwright MCP caps: ${invalid.join(", ")}`)
  return result as PlaywrightCapability[]
}

function parseViewport(value: string | undefined) {
  if (!value) return undefined
  const match = /^(\d+)x(\d+)$/.exec(value)
  if (!match) throw new Error(`Invalid viewport size "${value}". Expected WIDTHxHEIGHT, for example 1280x720.`)
  return { width: Number(match[1]), height: Number(match[2]) }
}

function browserConfig(value: McpPlaywrightArgs["browser"]): {
  browserName: "chromium" | "firefox" | "webkit"
  channel?: "chrome" | "msedge"
} {
  if (value === "firefox" || value === "webkit") return { browserName: value }
  if (value === "chrome" || value === "msedge") return { browserName: "chromium", channel: value }
  // Prefer the system Chrome channel by default. The bundled Playwright MCP
  // otherwise asks users to install its matching Chromium build at first use.
...
export const McpPlaywrightCommand = cmd<{}, McpPlaywrightArgs>({
  ...
  builder: (yargs) =>
    yargs
      ...
      .option("headed", { type: "boolean", describe: "run browser headed; overrides --headless" })
      .option("browser", {
        type: "string",
        default: "chrome" as const,
        choices: ["chromium", "chrome", "firefox", "webkit", "msedge"] as const,
        describe: "browser or Chromium channel to use",
      })
      .option("caps", {
        type: "string",
        default: "default",
        describe: `comma-separated capabilities; default enables ${DEFAULT_PLAYWRIGHT_CAPS.join(",")}; use all to include devtools`,
      })
      .option("output-dir", {
        type: "string",
      ...
      .option("viewport-size", { type: "string", describe: "browser viewport size, for example 1280x720" })
      ...
      .option("console-level", {
        ...
        describe: "console message level",
      })
      .option("allow-unrestricted-file-access", {
        type: "boolean",
        describe: "allow file access outside workspace roots",
      })
      .option("ignore-https-errors", { type: "boolean", describe: "ignore HTTPS errors" })
  ...
  async handler(args) {
    ...
    const server = await createConnection({
      browser: {
        ...
        contextOptions: {
          ...(args.viewportSize ? { viewport: parseViewport(args.viewportSize) } : {}),
          ...(args.ignoreHttpsErrors ? { ignoreHTTPSErrors: true } : {}),
          ...(args.blockServiceWorkers ? { serviceWorkers: "block" as const } : {}),
        },
      },
      ...
      console: args.consoleLevel ? { level: args.consoleLevel } : undefined,
      allowUnrestrictedFileAccess: args.allowUnrestrictedFileAccess,
      testIdAttribute: args.testIdAttribute,
      saveSession: args.saveSession,
      sharedBrowserContext: args.sharedBrowserContext,
      timeouts: {
        action: args.timeoutAction,
        navigation: args.timeoutNavigation,
      },
    })
...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in openchinacode.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .openchinacode/ subdirectory too)
  const candidates = [path.join(baseDir, "openchinacode.jsonc"), path.join(baseDir, "openchinacode.json")]

  if (!global) {
    candidates.push(
      path.join(baseDir, ".openchinacode", "openchinacode.jsonc"),
      path.join(baseDir, ".openchinacode", "openchinacode.json"),
    )
  }
  ...
  // Default to openchinacode.jsonc if none exist
  return candidates[0]
}
