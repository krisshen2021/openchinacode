...
//   2. Interactive local: boots the split-footer direct mode with an
//      in-process server.
//   3. Interactive attach: connects to a running server and runs interactive
//      mode against it.
//
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run OpenChinaCode with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running OpenChinaCode server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
      ...
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to OPENCHINACODE_SERVER_USERNAME or 'openchinacode')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      if (interactive && args.command) {
        die("interactive mode cannot be used with --command")
      }

      if (args.demo && !interactive) {
        die("--demo requires interactive mode")
      }

      if (interactive && args.format === "json") {
        die("interactive mode cannot be used with --format json")
      }

      if (args["replay-limit"] !== undefined && !interactive) {
        die("--replay-limit requires interactive mode")
      }
...
export async function runMini(input: MiniCommandInput) {
  if (!RunCommand.handler) throw new Error("Interactive command handler is unavailable")
