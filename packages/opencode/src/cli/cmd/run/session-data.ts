...
import { formatTaskRoutePart } from "./task-route"
...
type CompactionProgressProperties = {
  sessionID: string
  stage: string
  message: string
  model?: {
  ...
  profile?: {
    source: string
    risk: string
    profiles: Array<{ type: string; weight: number }>
    mustPreserve: string[]
  }
}
...
function formatCompactionModel(model: CompactionProgressProperties["model"]) {
  if (!model) return undefined
  return `${model.providerID}/${model.modelID}${model.variant ? `#${model.variant}` : ""}`
}

function formatCompactionProgress(progress: CompactionProgressProperties): string {
  const model = formatCompactionModel(progress.model)
  if (progress.stage === "judge_result" && progress.judge) {
    const judgeModel =
    ...
    return `Compaction judge: ${judgeModel ?? "unavailable"} -> ${progress.judge.status}${elapsed}${error}`
  }

  if (progress.stage === "profile_ready" && progress.profile) {
    const profiles = progress.profile.profiles
      .map((item) => `${item.type} ${Math.round(item.weight * 100)}%`)
      .join(", ")
  ...
  if (progress.stage === "route" && model) {
    return `Compaction route: ${model} · ${progress.message}`
  }

  if ((progress.stage === "summary_started" || progress.stage === "summary_finished") && model) {
    return `Compaction summary: ${model} · ${progress.message}`
  }

  return `Compaction: ${progress.message}`
}
...
function toolStatus(part: ToolPart): string {
  ...
  const route = formatTaskRoutePart(part)
  ...
  if (typeof desc === "string" && desc.trim()) {
    return [`running ${desc.trim()}`, route].filter(Boolean).join(" · ")
  }

  const type = state.input?.subagent_type
  if (typeof type === "string" && type.trim()) {
    return [`running ${type.trim()}`, route].filter(Boolean).join(" · ")
  }

  return ["running task", route].filter(Boolean).join(" · ")
}
...
export function reduceSessionData(input: SessionDataInput): SessionDataOutput {
  ...
  if (event.type === "session.compaction.progress") {
    if (event.properties.sessionID !== input.sessionID) {
      return out(data, commits)
    }
    ...
    commits.push({
      kind: "system",
      text: formatCompactionProgress(event.properties as CompactionProgressProperties),
      phase: "progress",
      source: "system",
    })
    return out(data, commits, patch({ status: "compacting" }))
  }
