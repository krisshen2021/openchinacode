...
//
// Renders the full OpenChinaCode entry logo and a compact [O] exit badge, plus
...
const BRAND_OPEN = "OPEN"
const BRAND_CHINA = "CHINA"
const BRAND_CODE = "CODE"
const BRAND_CHINA_PINK = RGBA.fromHex("#ff4fb8")
...
function build(input: SplashWriterInput, kind: "entry" | "exit", ctx: ScrollbackRenderContext): ScrollbackSnapshot {
  ...
  if (kind === "entry") {
    ...
    pushBrand(lines, body_left, top, right)
    if (input.detail) {
      push(
        lines,
        body_left,
  ...
  if (kind === "exit") {
    ...
    push(
      ...
      top + 1,
      `openchinacode run --interactive -s ${meta.session_id}`,
      right,
      undefined,
      TextAttributes.BOLD,
    )
