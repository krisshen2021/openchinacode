...
import { formatTaskRouteMetadata } from "./task-route"
...
function runTask(p: ToolProps<typeof TaskTool>): ToolInline {
  ...
  const route = formatTaskRouteMetadata(p.metadata)
  return {
    icon,
    title: desc || `${kind} Task`,
    description: [desc ? `${kind} Agent` : undefined, route].filter(Boolean).join(" · ") || undefined,
  }
}
...
function snapTask(p: ToolProps<typeof TaskTool>): ToolSnapshot {
  ...
  const route = formatTaskRouteMetadata(p.metadata)
  const rows = [desc || title, route].filter((item): item is string => Boolean(item))
...
function scrollTaskFinal(p: ToolProps<typeof TaskTool>): string {
  ...
  const route = formatTaskRouteMetadata(p.metadata)
  if (!row) {
    return [`# ${kind} Task`, route].filter(Boolean).join("\n")
  }

  return [`# ${kind} Task`, row, route].filter(Boolean).join("\n")
}
