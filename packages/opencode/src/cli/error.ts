...
export function FormatError(input: unknown): string | undefined {
  ...
  if (NamedError.hasName(input, "MCPFailed")) {
    ...
    return `MCP server "${data}" failed. Note, OpenChinaCode does not support MCP authentication yet.`
  }
  ...
  if (providerModelNotFound) {
    ...
    return [
      ...
      `Try: \`openchinacode models\` to list available models`,
      `Or check your config (openchinacode.json) provider/model names`,
    ].join("\n")
  }
  ...
  if (remoteAuth) {
    ...
    return [
      ...
      ...(url ? [`Run \`openchinacode auth login ${url}\` to re-authenticate.`] : []),
    ].join("\n")
  }
