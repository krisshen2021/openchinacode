...
describe("run session data", () => {
  ...
  test("surfaces compaction progress as system commits", () => {
    const out = reduce(createSessionData(), {
      id: "evt_compaction",
      ...
      properties: {
        sessionID: "session-1",
        stage: "judge_result",
        message: "Compaction judge returned a valid profile",
        judge: {
          status: "valid",
    ...
    expect(out.commits).toEqual([
      expect.objectContaining({
        kind: "system",
        text: "Compaction judge: zhipuai-pay2go/glm-5.2 -> valid (1234ms)",
      }),
    ])
    expect(out.footer?.patch?.status).toBe("compacting")
  })
})
