...
describe("ProviderTransform.options - OpenChina provider defaults", () => {
  const sessionID = "session-openchina"
  ...
  test("Kimi K2.7 enables preserved thinking and prompt cache key", () => {
    expect(
  ...
  test("GLM 5.2 defaults to high reasoning effort instead of provider max", () => {
    expect(
      ProviderTransform.options({
        model: createModel("zhipuai-pay2go", "glm-5.2"),
        sessionID,
      }),
    ).toMatchObject({
  ...
  test("DeepSeek V4 enables thinking without forcing effort by default", () => {
    expect(
      ProviderTransform.options({
        model: createModel("deepseek", "deepseek-v4-pro"),
        sessionID,
      }),
    ).toMatchObject({
...
describe("ProviderTransform.message - DeepSeek reasoning content", () => {
  ...
  test("DeepSeek without tool calls strips historical reasoning_content", () => {
...
describe("ProviderTransform.message - empty image handling", () => {
  ...
  test("should instruct non-vision models to delegate visual inspection", () => {
    const validBase64 =
      "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg=="
    const nonVisionModel = {
      ...mockModel,
      capabilities: {
    ...
    const msgs = [
      {
        role: "user",
        content: [
          { type: "text", text: "Check the BGM icon state in this screenshot." },
          { type: "image", image: `data:image/png;base64,${validBase64}` },
        ],
      },
    ] as any[]

    const result = ProviderTransform.message(msgs, nonVisionModel, {}) as any[]

    expect(result).toHaveLength(1)
    expect(result[0].content[0]).toEqual({ type: "text", text: "Check the BGM icon state in this screenshot." })
    expect(result[0].content[1].text).toContain("this model does not support image input")
    expect(result[0].content[1].text).toContain('task_kind="visual_check"')
    expect(result[0].content[1].text).toContain('subagent_type="general"')
    expect(result[0].content[1].text).toContain("Do not answer that you cannot inspect the image")
  })
...
describe("ProviderTransform OpenChina sampling and body rewrite", () => {
  const createModel = (providerID: string, apiID: string): any => ({
    id: `${providerID}/${apiID}`,
    providerID,
    api: {
      id: apiID,
  ...
  test("uses practical default output token budgets for OpenChina models", () => {
    expect(ProviderTransform.maxOutputTokens(createModel("zhipuai-pay2go", "glm-5.2"))).toBe(65_536)
  ...
  test("slides OpenChina output token budgets to maximum for long output requests", () => {
    const messages = [{ role: "user", content: "请生成完整实现，不要省略任何代码。" }]
    expect(
  ...
  test("uses current turn intent instead of stale long-output history", () => {
    const messages = [
      { role: "user", content: "请生成完整实现，不要省略任何代码。" },
      { role: "assistant", content: "好的。" },
      { role: "user", content: "解释一下 LSP 是什么" },
    ]
    expect(
      ProviderTransform.maxOutputTokens({
        model: createModel("zhipuai-pay2go", "glm-5.2"),
  ...
  test("slides OpenChina output token budgets to maximum for current coding tasks", () => {
    for (const content of ["修复这个 bug", "实现一个 provider 参数优化功能", "重构这块架构并补测试"]) {
      expect(
        ProviderTransform.maxOutputTokens({
          model: createModel("zhipuai-pay2go", "glm-5.2"),
  ...
  test("keeps GLM vision checks on default output budget even with coding signals", () => {
    const decision = ProviderTransform.maxOutputDecision({
      model: createModel("zhipuai-pay2go", "glm-5v-turbo"),
      agentMode: "build",
      toolCount: 1,
      messages: [{ role: "user", content: "看一下截图里的错误并诊断这个前端 bug" }],
    })

    expect(decision?.tokens).toBe(65_536)
    expect(decision?.level).toBe("default")
    expect(decision?.reasons).toEqual(["vision-default"])
    expect(decision?.needsJudge).toBe(false)
  })

  test("marks ambiguous auto-maxtokens llm turns for judging", () => {
    const decision = ProviderTransform.maxOutputDecision({
      model: createModel("zhipuai-pay2go", "glm-5.2"),
      autoMaxTokens: "llm",
      agentMode: "build",
      toolCount: 1,
      messages: [{ role: "user", content: "看一下 packages/core/src/config.ts 这块怎么样" }],
    })
    expect(decision?.tokens).toBe(65_536)
    expect(decision?.needsJudge).toBe(true)
  })

  test("slides OpenChina output token budgets to maximum for max variants", () => {
    expect(
      ProviderTransform.maxOutputTokens({
        model: createModel("zhipuai-pay2go", "glm-5.2"),
        variant: "max",
  ...
  test("GLM keeps only one sampling control when both are present", () => {
    expect(
      ProviderTransform.rewriteRequestBody(createModel("zhipuai-pay2go", "glm-5.2"), {
        model: "glm-5.2",
        temperature: 0.7,
        top_p: 0.9,
      }),
  ...
  test("Kimi K2.7 removes fixed params and converts max token field", () => {
    expect(
      ProviderTransform.rewriteRequestBody(createModel("moonshotai-cn", "kimi-k2.7-code"), {
  ...
  test("Kimi K2.7 keeps required thinking enabled even when upstream disables it", () => {
    expect(
      ProviderTransform.rewriteRequestBody(createModel("moonshotai-cn", "kimi-k2.7-code"), {
        model: "kimi-k2.7-code",
  ...
  test("DeepSeek V4 thinking requests remove ignored sampling params", () => {
    expect(
      ProviderTransform.rewriteRequestBody(createModel("deepseek", "deepseek-v4-pro"), {
  ...
  test("DeepSeek no-thinking requests keep explicit sampling params", () => {
    expect(
      ProviderTransform.rewriteRequestBody(createModel("deepseek", "deepseek-v4-pro"), {
        model: "deepseek-v4-pro",
        temperature: 0.6,
...
describe("ProviderTransform.variants", () => {
  ...
  test("DeepSeek V4 returns official thinking variants", () => {
    const model = createMockModel({
