...
export async function tmpdir<T>(options?: TmpDirOptions<T>) {
  ...
  if (options?.config) {
    ...
    await Bun.write(path.join(dirpath, "openchinacode.json"), text)
    await Bun.write(path.join(dirpath, "opencode.json"), text)
  }
...
export function tmpdirScoped<E = never, R = never>(options?: {
  ...
  return Effect.gen(function* () {
    ...
    if (options?.config) {
      ...
      const text = JSON.stringify({ $schema: "https://opencode.ai/config.json", ...resolved })
      yield* Effect.promise(() => fs.writeFile(path.join(dir, "openchinacode.json"), text))
      yield* Effect.promise(() => fs.writeFile(path.join(dir, "opencode.json"), text))
    }
