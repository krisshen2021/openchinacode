...
describe("tool.shell", () => {
  ...
  each("blocks long-running service commands before permission", () =>
    Effect.gen(function* () {
      ...
      yield* runIn(
        ...
        Effect.gen(function* () {
          ...
          const result = yield* run({ command: "npx tsx server/index.ts" }, capture(requests))
          expect(requests).toEqual([])
          expect(result.metadata.exit).toBe(null)
          expect(result.output).toContain("process_start")
        }),
      )
    }),
  )

  each("blocks broad process-kill commands before permission", () =>
    Effect.gen(function* () {
      const tmp = yield* tmpdirScoped()
      yield* runIn(
        tmp,
        Effect.gen(function* () {
          const requests: Array<Omit<PermissionV1.Request, "id" | "sessionID" | "tool">> = []
          const result = yield* run({ command: 'pkill -9 -f "tsx server/index"' }, capture(requests))
          expect(requests).toEqual([])
          expect(result.metadata.exit).toBe(null)
          expect(result.output).toContain("process_stop")
        }),
      )
    }),
  )
