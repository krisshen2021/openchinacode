...
describe("tool.task", () => {
  ...
  it.instance(
    "routes subagents through task_policy when no explicit model is set",
    () =>
      Effect.gen(function* () {
        const { chat, assistant } = yield* seed()
        const tool = yield* TaskTool
        ...
        expect(result.metadata.modelSource).toBe("task_policy.routes")
        expect(result.metadata.taskPolicy?.kind).toBe("debug")
        expect(result.metadata.taskPolicy?.complexity).toBe("medium")
        expect(result.metadata.taskPolicy?.variant).toBe("max")
      }),
    {
      config: {
        ...testProviderConfig(),
