...
describe("tool.read truncation", () => {
  ...
  it.live("rejects browser video files instead of attaching or reading them", () =>
    Effect.gen(function* () {
      const dir = yield* tmpdirScoped()
      const webm = Buffer.from([0x1a, 0x45, 0xdf, 0xa3, 0x9f, 0x42, 0x86, 0x81, 0x01])
      yield* put(path.join(dir, "bgm-animation-check.webm"), webm)

      const err = yield* fail(dir, { filePath: path.join(dir, "bgm-animation-check.webm") })
      expect(err.message).toContain("Cannot read video file")
      expect(err.message).toContain("browser telemetry")
    }),
  )
