...
function routedProvider() {
  const parent = createModel({ context: 100_000, output: 32_000 })
  ...
  return ProviderTest.fake({
    model: parent,
    info: providers[parent.providerID],
    list: () => Effect.succeed(providers),
    getProvider: (providerID) => {
      const item = providers[providerID]
      return item ? Effect.succeed(item) : Effect.die(new Error(`Unknown test provider: ${providerID}`))
    },
...
function rejectingJudgeProvider(error: unknown) {
  const language = {
...
function fake(
  ...
  capture?: {
    create?: (input: Parameters<SessionProcessorModule.SessionProcessor.Interface["create"]>[0]) => void
    process?: (input: Parameters<SessionProcessorModule.SessionProcessor.Handle["process"]>[0]) => void
  },
) {
  ...
  return {
    ...
    process: Effect.fn("TestSessionProcessor.process")((input) =>
      Effect.sync(() => capture?.process?.(input)).pipe(Effect.as(result)),
    ),
...
describe("session.compaction.process", () => {
  ...
  itCompaction.instance(
    "falls back to inferred profile when profile judge aborts",
    () => {
      ...
      }).pipe(withCompaction({ provider: rejectingJudgeProvider(abort) }))
    },
    { git: true },
  )
  ...
  itCompaction.instance(
    ...
    Effect.gen(function* () {
      ...
      const msg = yield* createUserMessage(session.id, "hello")
      const msgs = yield* ssn.messages({ sessionID: session.id })
      let created: Parameters<SessionProcessorModule.SessionProcessor.Interface["create"]>[0] | undefined
      let processed: Parameters<SessionProcessorModule.SessionProcessor.Handle["process"]>[0] | undefined
  ...
  itCompaction.instance(
    "builds profile-aware mixed compaction prompt",
    Effect.gen(function* () {
      const ssn = yield* SessionNs.Service
      const session = yield* ssn.create({})
      ...
      const prompt = modelMessageText(processed?.messages.at(-1))
      expect(result).toBe("continue")
      expect(prompt).toContain("<compaction-profile-json>")
      expect(prompt).toContain('"type": "debug_trace"')
      expect(prompt).toContain('"type": "implementation_state"')
      expect(prompt).toContain('"type": "architecture_memory"')
  ...
  itCompaction.instance(
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        expect(captured).toContain("<compaction-profile-json>")
        expect(captured).toContain("## Critical Continuity Facts")
        expect(captured).toContain("## General Context")
        expect(captured).toContain("## Next Actions")
      }).pipe(withCompaction({ llm: stub.llmLayer }))
    },
    { git: true },
  )
