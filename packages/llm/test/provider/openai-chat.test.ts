...
describe("OpenAI Chat route", () => {
  ...
  it.effect("passes provider-scoped thinking options through OpenAI-compatible chat bodies", () =>
    Effect.gen(function* () {
      const glm = OpenAIChat.route
        .with({ provider: "zhipuai-pay2go", endpoint: { baseURL: "https://open.bigmodel.cn/api/paas/v4" } })
        .model({ id: "glm-5v-turbo" })
      const prepared = yield* LLMClient.prepare<OpenAIChat.OpenAIChatBody>(
        LLM.request({
          model: glm,
          messages: [Message.user("Look carefully.")],
          providerOptions: {
            "zhipuai-pay2go": {
      ...
      expect(prepared.body.thinking).toEqual({ type: "enabled", clear_thinking: true })
    }),
  )
