...
const I18N = {
  en: {
    ...
    "meta.manual.desc":
      "OpenChinaCode usage and configuration manual for GLM, Kimi, DeepSeek, task routing, compaction, LSP, and Playwright MCP.",
    ...
    "features.compaction.desc":
      "Manual or automatic compaction keeps a general summary, extracts the active task at higher granularity, and can retain raw recent turns with <code>/compact keep N</code>.",
    "features.budgets.title": "Model-aware token budgets",
    "features.budgets.desc":
      "<code>/auto-maxtokens</code> uses official model windows, task signals, and overflow checks so complex coding turns can request large outputs without needless compression.",
    "features.tuning.title": "China-provider tuning",
    "features.tuning.desc":
    ...
    "features.browser.desc":
      "<code>/test-mcp</code>, <code>/browser-check</code>, and <code>/integration-test</code> wire Playwright MCP and test reports into the agent workflow.",
    ...
    "media.nl.desc":
      "Just describe what you want, or run <code>/image-generate</code> and <code>/video-generate</code> with an optional prompt. Missing details trigger follow-up questions, not guesses.",
    ...
    "ocr.auth.desc":
      "Run <code>/ocr-auth</code> in the TUI, or set <code>BAIDU_OCR_API_KEY</code> and <code>BAIDU_OCR_SECRET_KEY</code> environment variables. Keys are stored in <code>~/.local/share/openchinacode/auth.json</code> under provider <code>baidu-unlimited-ocr</code>.",
    "ocr.formats.tag": "Formats",
    "ocr.formats.title": "Supported file types",
    "ocr.formats.desc":
      "Documents: <code>pdf</code>, <code>ofd</code>, <code>doc</code>, <code>docx</code>, <code>txt</code>, <code>wps</code>, <code>ppt</code>, <code>pptx</code>. Images: <code>jpg</code>, <code>jpeg</code>, <code>png</code>, <code>bmp</code>, <code>tif</code>, <code>tiff</code>. Output saved to <code>/tmp/openchinacode/ocr</code> as Markdown + JSON + metadata.",
    "ocr.nl.tag": "Trigger",
    "ocr.nl.title": "Paste or slash command",
    "ocr.nl.desc":
      "Run <code>/ocr</code> with optional file paths, or <code>Ctrl+V</code> paste a document into the TUI. Document files always route to OCR; images only route to OCR when you explicitly ask to extract text, parse tables, or convert to Markdown. Regular screenshots and UI images still go to the visual model.",

    /* ── CTA ── */
    "cta.title": "Need the exact commands and config files?",
    "cta.desc":
      "The manual documents slash commands, provider auth, task policy overrides, smart compaction, LSP, Playwright MCP, and debugging logs.",
    ...
    "config.auth.desc":
      "Written by <code>openchinacode providers login</code>. File mode is restricted when written by the CLI.",
    ...
    "commands.desc":
      "OpenChinaCode-specific commands are grouped under <code>OpenChinaCode</code> in the TUI command palette.",
    ...
    "commands.taskpolicy.effect":
      "Opens the local TUI task-route table. <code>on</code>/<code>off</code> hot-toggles the entire task policy and task subagent entry; <code>extra-on</code>/<code>extra-off</code> toggles the extra task router; <code>extra-status</code> shows current state. Does not call the model or inject the table into conversation.",
    "commands.soul.effect":
      "Selects the conversation persona. <code>custom</code> saves to <code>.openchinacode/souls/custom.md</code> and writes <code>soul.active</code> + <code>soul.custom_path</code> to project config. Refreshes the instance for new turns. Does not call the model.",
    "commands.taskclassify.effect":
    ...
    "commands.integrationtest.effect":
    ...
    "commands.imagegen.effect":
      "Opens the ByteDance Seedream 5.0 Pro image generation wizard with optional inline prompt.",
    "commands.videogen.effect":
      "Opens the ByteDance Seedance 2.0 Mini video generation wizard with optional inline prompt.",
    "commands.ocrauth.effect":
      "Saves Baidu Unlimited-OCR API Key and Secret Key for <code>ocr_extract</code>. Stored in <code>~/.local/share/openchinacode/auth.json</code> under provider <code>baidu-unlimited-ocr</code>. Env fallback: <code>BAIDU_OCR_API_KEY</code> + <code>BAIDU_OCR_SECRET_KEY</code>.",
    "commands.ocr.effect":
      "Opens the Baidu Unlimited-OCR document parsing wizard. Supports PDF, OFD, DOC, DOCX, TXT, WPS, PPT, PPTX, and image formats. Results saved to <code>/tmp/openchinacode/ocr</code> as Markdown + JSON + metadata.",
    "commands.sessions.effect":
      "Opens the session picker. Use arrow keys to switch between current-project and all-project sessions.",
    "commands.permissions.effect":
      "Opens the project permission policy panel. Choose Trust All, Safe, Ask Everything, Readonly, or Reset. Writes to <code>./.openchinacode/openchinacode.jsonc</code> and applies a runtime override immediately.",
    ...
    "routing.overrideDesc":
      "Put this in <code>~/.config/openchinacode/openchinacode.jsonc</code> or in a project-level OpenChinaCode config.",

    /* ── Extra Task Router ── */
    "routing.extra.title": "Extra Task Router",
    "routing.extra.desc":
      "When enabled, ordinary prompts pass through a fast LLM judge that can automatically insert a routed subtask — no explicit <code>/task</code> call needed. Disabled by default.",
    "routing.extra.base.tag": "Base",
    "routing.extra.base.title": "System-level base routing",
    "routing.extra.base.desc":
    ...
    "routing.extra.control.desc":
      "Use <code>/task-policy extra-on</code> / <code>extra-off</code> / <code>extra-status</code>, or set <code>task_policy.extra_router.enabled</code> in config.",
    ...
    "compaction.desc":
      "OpenChinaCode compaction is routed through task policy and defaults to <code>moonshotai-cn/kimi-k3#high</code>. It uses three layers: general summary, active-task extraction, and minimal raw recent tail.",
    "compaction.strategy.tag": "Strategy",
    "compaction.strategy.title": "Profile-aware summary",
    "compaction.strategy.desc":
    ...
    "testing.lsp.desc":
      "Use <code>/lsp on</code>. When enabled, language-server diagnostics can be fed into the model during code edits.",
    "testing.mcp.tag": "MCP",
    "testing.mcp.title": "Playwright from the TUI",
    "testing.mcp.desc":
      "Use <code>/test-mcp on</code>. This writes Playwright MCP config and hot-connects it without leaving OpenChinaCode.",
    "testing.vision.tag": "Vision",
    "testing.vision.title": "Screenshot interpretation",
    "testing.vision.desc":
      "Visual checks route to <code>zhipuai-pay2go/glm-5v-turbo</code>. Animation checks should prefer DOM/CSS telemetry and pixel diffs.",
    ...
    "sessions.desc":
      "Open the session picker with <code>/sessions</code> (aliases <code>/resume</code>, <code>/continue</code>). Use arrow keys to switch scope.",
    "sessions.scope.tag": "Scope",
    "sessions.scope.title": "Arrow-key scope switch",
    "sessions.scope.desc":
      "Press <code>←</code> / <code>→</code> to toggle between <strong>Current Project</strong> (with pin/unpin) and <strong>All Projects</strong> (global session list).",
    ...
    "media.auth.desc":
      "Run <code>/media-auth</code> in the TUI, or set the <code>ARK_API_KEY</code> environment variable. The key is stored in <code>~/.local/share/openchinacode/auth.json</code> under provider <code>volcengine-ark</code>.",
    "media.image.tag": "Image",
    "media.image.manual.title": "ByteDance Seedream 5.0 Pro",
    "media.image.manual.desc":
  ...
  zh: {
    ...
    "meta.manual.desc":
      "OpenChinaCode 使用与配置手册，涵盖 GLM、Kimi、DeepSeek、任务路由、压缩、LSP 和 Playwright MCP。",
    ...
    "hero.lead":
      "精简的终端编程智能体，基于 opencode 分支构建，围绕中国模型路由、智能压缩、滑动输出预算、人民币成本追踪、LSP 诊断和 Playwright 浏览器测试重新打造。",
    "hero.install": "立即安装",
    "hero.copied": "已复制！",
    ...
    "providers.lead":
      "OpenChinaCode 保留了 opencode 终端优先的工作流，然后将默认模型范围收窄至 GLM、Kimi 和 DeepSeek，配合模型感知的请求转换、任务路由、压缩、测试和人民币成本显示。",
    ...
    "features.routing.desc":
      "规划、架构、重构、审查、实现、探索、调试、测试修复、摘要、压缩和视觉检查各有快速/中等/复杂路由。",
    "features.compaction.title": "智能压缩管线",
    "features.compaction.desc":
      "手动或自动压缩保留通用摘要，以更高粒度提取活跃任务，并可通过 <code>/compact keep N</code> 保留原始近期轮次。",
    "features.budgets.title": "模型感知 Token 预算",
    "features.budgets.desc":
      "<code>/auto-maxtokens</code> 使用官方模型窗口、任务信号和溢出检查，使复杂编程轮次可以请求大输出而无需不必要的压缩。",
    "features.tuning.title": "中国提供商调优",
    "features.tuning.desc":
      "GLM、Kimi 和 DeepSeek 使用直连官方 OpenAI 兼容 API，具有提供商特定的最大 token、推理、采样和工具调用行为。",
    "features.browser.title": "浏览器测试工作流",
    "features.browser.desc":
      "<code>/test-mcp</code>、<code>/browser-check</code> 和 <code>/integration-test</code> 将 Playwright MCP 和测试报告接入智能体工作流。",
    ...
    "media.nl.desc":
      "直接描述需求，或运行 <code>/image-generate</code> 和 <code>/video-generate</code> 并附加可选提示词。缺少关键参数时会追问，而非随意猜测。",
    ...
    "ocr.hero.desc": "复制任意支持的文档，粘贴到 OpenChinaCode 输入框中，即可 OCR 扫描识别为结构化文字。",
    "ocr.hero.cta": "阅读手册",
    ...
    "ocr.section.desc":
      "OpenChinaCode 集成百度 Unlimited-OCR，提供云端原生文档解析。提交 PDF、Office 文档或图片，即可获得结构化的 Markdown 和 JSON — 无需本地 GPU 或推理环境。",
    "ocr.auth.tag": "认证",
    "ocr.auth.title": "百度 Unlimited-OCR 凭证",
    "ocr.auth.desc":
      "在 TUI 中运行 <code>/ocr-auth</code>，或设置 <code>BAIDU_OCR_API_KEY</code> 和 <code>BAIDU_OCR_SECRET_KEY</code> 环境变量。密钥存储在 <code>~/.local/share/openchinacode/auth.json</code> 中，provider 为 <code>baidu-unlimited-ocr</code>。",
    "ocr.formats.tag": "格式",
    "ocr.formats.title": "支持的文件类型",
    "ocr.formats.desc":
      "文档：<code>pdf</code>、<code>ofd</code>、<code>doc</code>、<code>docx</code>、<code>txt</code>、<code>wps</code>、<code>ppt</code>、<code>pptx</code>。图片：<code>jpg</code>、<code>jpeg</code>、<code>png</code>、<code>bmp</code>、<code>tif</code>、<code>tiff</code>。输出保存到 <code>/tmp/openchinacode/ocr</code>，包含 Markdown + JSON + 元数据。",
    "ocr.nl.tag": "触发",
    "ocr.nl.title": "粘贴或斜杠命令",
    "ocr.nl.desc":
      "运行 <code>/ocr</code> 并传入文件路径，或在 TUI 中 <code>Ctrl+V</code> 粘贴文档。文档文件始终走 OCR；图片仅在你明确要求提取文字、解析表格或转 Markdown 时才走 OCR。普通截图和 UI 图片仍走视觉模型。",
    ...
    "manual.lead":
      "本页涵盖 OpenChinaCode 专属工作流：GLM/Kimi/DeepSeek 提供商配置、自定义斜杠命令、任务策略路由、智能压缩、LSP 诊断和 Playwright 测试。",
    ...
    "commands.soul.effect":
      "选择对话人格。<code>custom</code> 保存到 <code>.openchinacode/souls/custom.md</code>，并写入 <code>soul.active</code> + <code>soul.custom_path</code> 到项目配置。刷新实例，对新 turn 生效。不调用模型。",
    ...
    "commands.integrationtest.effect":
      "提示智能体使用 OpenChinaCode 集成测试工作流：运行、检查报告、定义缺陷、修复、重跑。",
    ...
    "commands.ocrauth.effect":
      "保存百度 Unlimited-OCR API Key 和 Secret Key，供 <code>ocr_extract</code> 使用。存储于 <code>~/.local/share/openchinacode/auth.json</code>，provider 为 <code>baidu-unlimited-ocr</code>。环境变量兜底：<code>BAIDU_OCR_API_KEY</code> + <code>BAIDU_OCR_SECRET_KEY</code>。",
    "commands.ocr.effect":
      "打开百度 Unlimited-OCR 文档解析向导。支持 PDF、OFD、DOC、DOCX、TXT、WPS、PPT、PPTX 及图片格式。结果保存到 <code>/tmp/openchinacode/ocr</code>，包含 Markdown + JSON + 元数据。",
    "commands.sessions.effect": "打开 session 选择器。使用方向键在当前项目和全部项目 session 之间切换。",
    "commands.permissions.effect":
      "打开项目权限策略面板。可选 Trust All、Safe、Ask Everything、Readonly 或 Reset。写入 <code>./.openchinacode/openchinacode.jsonc</code> 并立即下发 runtime override 生效。",

    /* ── Task Routing ── */
    "routing.title": "任务路由策略",
    "routing.desc":
      "子智能体按任务类型和复杂度路由。通用任务继承父模型；专业任务使用预设的 GLM/Kimi/DeepSeek 默认配置。",
    ...
    "routing.overrideDesc":
      "将其放入 <code>~/.config/openchinacode/openchinacode.jsonc</code> 或项目级 OpenChinaCode 配置中。",

    /* ── Extra Task Router ── */
    "routing.extra.title": "Extra 任务路由",
    "routing.extra.desc":
      "开启后，普通 prompt 会先经过快速 LLM 判定，适合时自动插入路由子任务——无需显式调用 <code>/task</code>。默认关闭。",
    "routing.extra.base.tag": "基础",
    "routing.extra.base.title": "系统级基础路由",
    "routing.extra.base.desc":
      "显式 task/subagent 调用触发内置路由表 — GLM 负责规划，Kimi 负责速度，DeepSeek 负责调试。",
    "routing.extra.smart.tag": "Extra",
    "routing.extra.smart.title": "Extra 智能分发",
    "routing.extra.smart.desc":
      "快速评判模型（DeepSeek V4 Flash）检查每个普通 prompt，适合委派时自动插入带 task_kind + complexity 的子任务。",
    "routing.extra.control.tag": "控制",
    "routing.extra.control.title": "开关与配置",
    "routing.extra.control.desc":
      "使用 <code>/task-policy extra-on</code> / <code>extra-off</code> / <code>extra-status</code>，或在配置中设置 <code>task_policy.extra_router.enabled</code>。",
    "routing.extra.config":
      '{\n  "task_policy": {\n    "extra_router": {\n      "enabled": true,\n      "confidence_threshold": 0.7,\n      "allow": ["plan", "refactor", "review", "debug", "test_fix"],\n      "deny": ["general", "summarize", "compaction"]\n    }\n  }\n}',
    ...
    "compaction.desc":
      "OpenChinaCode 压缩通过任务策略路由，默认使用 <code>moonshotai-cn/kimi-k3#high</code>。它使用三层：通用摘要、活跃任务提取和最小原始近期尾部。",
    "compaction.strategy.tag": "策略",
    "compaction.strategy.title": "档案感知摘要",
    "compaction.strategy.desc":
      "评判模型首先输出稳定的 JSON 档案，然后 OpenChinaCode 根据该档案构建固定的 Markdown 压缩提示词。",
    ...
    "testing.vision.desc":
      "视觉检查路由到 <code>zhipuai-pay2go/glm-5v-turbo</code>。动画检查应优先使用 DOM/CSS 遥测和像素差异。",
    ...
    "sessions.desc":
      "使用 <code>/sessions</code>（别名 <code>/resume</code>、<code>/continue</code>）打开 session 选择器。使用方向键切换范围。",
    "sessions.scope.tag": "范围",
    "sessions.scope.title": "方向键切换范围",
    "sessions.scope.desc":
      "按 <code>←</code> / <code>→</code> 在<strong>当前项目</strong>（支持 pin/unpin）和<strong>全部项目</strong>（全局 session 列表）之间切换。",
    ...
    "media.manual.desc":
      "OpenChinaCode 可通过火山方舟生成图片和视频，由字节跳动 Seedream 5.0 Pro 和 Seedance 2.0 Mini 驱动。",
    "media.auth.tag": "认证",
    "media.auth.title": "火山方舟凭证",
    "media.auth.desc":
      "在 TUI 中运行 <code>/media-auth</code>，或设置 <code>ARK_API_KEY</code> 环境变量。密钥存储在 <code>~/.local/share/openchinacode/auth.json</code> 中，提供商为 <code>volcengine-ark</code>。",
    "media.image.tag": "生图",
    "media.image.manual.title": "字节跳动 Seedream 5.0 Pro",
    "media.image.manual.desc":
      "最高 2K 分辨率，1:1 至 21:9 宽高比，最多 10 张参考图（本地路径、URL 或 data URI），PNG 或 JPEG 输出。",
    "media.image.cmd": "/image-generate\n/image-generate 一只赛博朋克猫 mascot，粉色霓虹，干净矢量风",
    "media.video.tag": "生视频",
    "media.video.manual.title": "字节跳动 Seedance 2.0 Mini",
    "media.video.manual.desc":
      "4–15 秒短片，480p 或 720p，自适应至 21:9 比例，最多 9 张参考图，3 个参考视频 URL，严格首尾帧控制，生成音频。",
    "media.video.cmd": "/video-generate\n/video-generate 一个 5 秒项目宣传短片，镜头缓慢推进，科技感但不浮夸",
    "media.nl.tag": "触发",
    "media.nl.title": "自然语言或斜杠命令",
    "media.nl.manual.desc":
      "用自然语言描述需求，或运行 <code>/image-generate</code> / <code>/video-generate</code> 并附加可选提示词。缺少关键参数时，智能体会追问而非猜测。生成文件会立即下载到 <code>/tmp/openchinacode/media/images</code> 和 <code>/tmp/openchinacode/media/videos</code>，并写入同名 metadata JSON。",
  },
}
