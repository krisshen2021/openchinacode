...
| `kimi-k3`                  |  20.5 | 101.25 |       2.05 | CNY / 1M tokens |
...
Kimi K3 官方价格是美元：cache hit `$0.30 / MTok`、input `$3.00 / MTok`、output `$15.00 / MTok`。OpenChinaCode 的 UI 统一显示 CNY，所以 `kimi-k3` 使用和既有 K2.7 条目一致的 Moonshot 折算基准写入价格表。
...
- Kimi K2.7 code 保留旧兼容：开启 `thinking.keep = "all"`，使用 `prompt_cache_key = sessionID`，非 `auto` / `none` 的 `tool_choice` 会降为 `auto`。
- Kimi K3 不注入 K2 thinking，也不注入 `prompt_cache_key`。
- Kimi K3 支持 variant：`high`、`max`，分别发送 `reasoning_effort=high/max`。
- Kimi K3 保留 `tool_choice=required`，因为官方支持 `auto` / `none` / `required`。
- Kimi K3 固定官方采样参数：`temperature=1.0`、`top_p=0.95`、`n=1`、presence/frequency penalty 为 0；OpenChinaCode 不主动传这些字段。
...
`/ocr-auth` 是本地 TUI command，不调用当前对话模型。它保存百度 Unlimited-OCR API Key 和 Secret Key 到 auth。

```text
/ocr-auth
/ocr-auth <API_KEY> <SECRET_KEY>
```

`/ocr` 打开 OCR 文件选择向导。它会生成一段合成 prompt，要求当前模型调用 `ocr_extract`。

```text
/ocr
/ocr /path/to/file.pdf
/ocr /path/a.pdf,/path/b.docx
```
...
目标：用户在 TUI prompt 中 Ctrl+V 粘贴图片后，不依赖关键词判断；如果当前主模型支持 image input，则直接把图片提交给主模型，否则用 GLM-5V 做稳定视觉预处理，再把结果交给主模型继续编程任务。
...
- `packages/tui/src/component/prompt/index.tsx`：
  ...
  - `shouldUseVisualPreprocess()`：基于当前模型 metadata 判断是否需要 GLM-5V 预处理。
...
-> If current model supports image input: send original prompt + image file part directly
-> Else: user message includes original prompt text + visual preprocessing subtask
...
- 对 Kimi K3 / GLM-5V 等视觉模型，图片 file part 直接送给主模型。
- 对 DeepSeek / GLM-5.2 等非视觉模型，图片 file part 不再直接送给主模型，避免 unsupported image input 后走偏。
...
### `/ocr-auth`, `/ocr`, `ocr_extract`

原生 OCR / 文档解析入口。云端默认走百度 Unlimited-OCR，不依赖用户本地 HuggingFace/vLLM/SGLang 推理环境。
...
- `packages/opencode/src/tool/ocr-common.ts`
- `packages/opencode/src/tool/ocr-extract.ts`
- `packages/opencode/src/tool/ocr-extract.txt`
- `packages/opencode/src/tool/registry.ts`
...
- `packages/tui/src/component/prompt/ocr-route.ts`
- `packages/tui/src/component/prompt/index.tsx`

认证：

```text
provider id: baidu-unlimited-ocr
env fallback: BAIDU_OCR_API_KEY + BAIDU_OCR_SECRET_KEY
auth file: ~/.local/share/openchinacode/auth.json
```

`/ocr-auth` 把 API Key 存入 auth `key`，把 Secret Key 存入 `metadata.secret_key`。

百度接口：

```text
token: POST/GET https://aip.baidubce.com/oauth/2.0/token
submit: POST https://aip.baidubce.com/rest/2.0/brain/online/v2/unlimited-ocr-parser/task
query:  POST https://aip.baidubce.com/rest/2.0/brain/online/v2/unlimited-ocr-parser/task/query
```

调用形态：

```text
file path / file:// / HTTP(S) URL / data URL
-> resolveOcrSource()
-> submitBaiduOcrTask()
-> poll queryBaiduOcrTask()
-> download markdown_url + parse_result_url
-> save to /tmp/openchinacode/ocr
```

支持格式严格按百度 Unlimited-OCR API 文档白名单：

- 文档：`pdf`、`ofd`、`doc`、`docx`、`txt`、`wps`、`ppt`、`pptx`
- 图片 OCR：`jpg`、`jpeg`、`png`、`bmp`、`tif`、`tiff`

不把 `xlsx` 加入 MVP 白名单，除非后续确认 Unlimited-OCR API 参数文档明确支持。

附件路由顺序：

```text
Ctrl+V image bytes -> image attachment
Ctrl+V file manager list -> local attachments
...
```text
/tmp/openchinacode/ocr/*.md
/tmp/openchinacode/ocr/*.parse.json
/tmp/openchinacode/ocr/*.metadata.json
```
...
```jsonc
{
  "task_policy": {
    ...
    "judges": {
      ...
      "compaction_profile": {
        "models": ["zhipuai-pay2go/glm-5.2", "moonshotai-cn/kimi-k3", "deepseek/deepseek-v4-flash"],
        "timeout_ms": 60000,
      },
      "task_router": {
        "models": ["deepseek/deepseek-v4-flash"],
...
- 默认 `/compact` 使用 `compaction.medium -> moonshotai-cn/kimi-k3#high`；`compaction.quick -> zhipuai-pay2go/glm-5.2#high`，`compaction.complex -> moonshotai-cn/kimi-k3#high`。
