...
- 默认开启任务完成提醒：session 完成、需要授权、需要用户回答、出错时会触发 TUI attention。

Kitty 等终端的完成提示音主要依赖 terminal bell，也就是 BEL 字符 `\u0007`。OpenChinaCode 默认开启：
...
```text
compaction.quick -> zhipuai-pay2go/glm-5.2#high
compaction.medium/complex -> moonshotai-cn/kimi-k3#high
```
...
在 TUI 输入框里 Ctrl+V 粘贴图片后，OpenChinaCode 会先把图片保存到本地临时目录，然后按当前主模型 metadata 自动选择处理方式：

- 当前主模型支持图片输入，例如 `moonshotai-cn/kimi-k3`：图片 file part 直接提交给主模型。
- 当前主模型不支持图片输入，例如 `zhipuai-pay2go/glm-5.2`、`deepseek/deepseek-v4-pro`：自动调用 GLM-5V 做视觉识别，识别结果会作为上下文喂给当前主模型。
...
如果当前主模型不支持图片输入，运行时会先出现一个视觉预处理 subtask：
...
然后主模型会基于 GLM-5V 的观察继续分析、改代码或给方案。这样即使当前主模型是 DeepSeek / GLM-5.2 / 其它非视觉模型，也能稳定理解截图内容。Kimi K3 这类视觉模型不会多走一次 subagent。
...
OpenChinaCode 新增了百度 Unlimited-OCR 云端文档解析工具：

- `ocr_extract`：提交文档或图片到 Baidu Unlimited-OCR，轮询结果，下载 Markdown 和 JSON。

它不是普通 LLM provider，也不依赖用户本机 GPU 或本地推理环境。适合处理 PDF、OFD、Word、PPT、WPS、TXT、扫描件、表格、合同、发票等“提取文字 / 转 Markdown / 解析文档结构”的任务。
...
| `review`       | `moonshotai-cn/kimi-k3#high`    | `zhipuai-pay2go/glm-5.2#high`   | `zhipuai-pay2go/glm-5.2#max`   |
| `implement`    | `moonshotai-cn/kimi-k3#high`    | `zhipuai-pay2go/glm-5.2#high`   | `zhipuai-pay2go/glm-5.2#max`   |
| `explore`      | `moonshotai-cn/kimi-k3#high`    | `moonshotai-cn/kimi-k3#high`    | `zhipuai-pay2go/glm-5.2#max`   |
...
| `summarize`    | `moonshotai-cn/kimi-k3#high`    | `zhipuai-pay2go/glm-5.2#high`   | `zhipuai-pay2go/glm-5.2#max`   |
| `compaction`   | `zhipuai-pay2go/glm-5.2#high`   | `moonshotai-cn/kimi-k3#high`    | `moonshotai-cn/kimi-k3#high`   |
...
普通自然语言输入只要包含 Ctrl+V 粘贴的图片，OpenChinaCode 会按当前主模型能力自动处理：

- 如果当前模型 metadata 标记 `capabilities.attachment=true` 且 `capabilities.input.image=true`，直接把图片 part 提交给当前模型，例如 Kimi K3。
- 否则先用 GLM-5V 做一次视觉预处理，再把识别结果交给当前主模型，例如 GLM-5.2 / DeepSeek V4 Pro。
...
保存百度 Unlimited-OCR 的 API Key 和 Secret Key，供 `ocr_extract` 使用。
...
```bash
export BAIDU_OCR_API_KEY="your-baidu-ocr-api-key"
export BAIDU_OCR_SECRET_KEY="your-baidu-ocr-secret-key"
```
...
```text
/ocr
/ocr /home/kris/Documents/bibleG15.pdf
```

支持格式按百度 Unlimited-OCR API 白名单收窄：
...
| `/task-policy off`          | 热禁用 OpenChinaCode 智能路由；原生 task/subagent 仍可用并继承父模型 |
...
`/task-policy on/off` 控制 OpenChinaCode 智能 task policy，适合在某些任务里关闭自动模型改派。关闭后，新 turn 不会注入完整的 OpenChinaCode task routing 提示，普通 subagent 不会走 GLM/Kimi/DeepSeek 策略表，而是使用父模型或 agent 默认模型。它不会禁用 opencode 原生 `task` 子代理能力。视觉检查是能力兜底例外：当当前主模型不能直接看图，`task_kind=visual_check` 仍可用，并继续路由到 GLM-5V。这个开关会写入全局配置，并热应用到当前运行实例的新 turn。
...
- `enabled: false` 会关闭 OpenChinaCode 智能 task policy 和普通 subagent 的模型改派；也可以用 `/task-policy off/on` 热切换。它不会禁用原生 `task` 子代理，普通子代理会继承父模型或使用 agent 默认模型。视觉检查 `visual_check` 是能力兜底例外，仍可路由到 GLM-5V。
